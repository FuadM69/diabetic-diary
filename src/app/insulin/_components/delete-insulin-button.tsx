...
function DeleteConfirmForm({
  ...
  const [state, formAction, isPending] = useActionState(
    async (_prev: InsulinActionResult, formData: FormData) =>
      deleteInsulinEntryAction(formData),
    initialState
  );
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <p className="text-sm text-white/70">
        Удалить эту запись? Действие нельзя отменить.
      </p>
      {state.error ? (
...
type DeleteInsulinButtonProps = {
  entryId: string;
};

export function DeleteInsulinButton({ entryId }: DeleteInsulinButtonProps) {
