...
function DeleteConfirmForm({
  ...
  const [state, formAction, isPending] = useActionState(
    async (_prev: FoodActionResult, fd: FormData) =>
      deleteFoodProductAction(fd),
    initialState
  );
...
export function DeleteFoodProductButton({ productId }: DeleteFoodProductButtonProps) {
    ...
    <>
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-2">
          <h3 className="text-base font-medium text-white">Удаление продукта</h3>
          <button
            type="button"
            onClick={close}
